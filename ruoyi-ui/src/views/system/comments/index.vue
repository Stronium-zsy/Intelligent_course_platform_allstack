<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="${comment}" prop="postId">
        <el-input
      ...
      <el-form-item label="${comment}" prop="commentIndex">
        <el-input
          v-model="queryParams.commentIndex"
          placeholder="请输入${comment}"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <el-form-item label="${comment}" prop="commentTime">
      ...
      <el-form-item label="${comment}" prop="likesCount">
        <el-input
      ...
      <el-form-item label="${comment}" prop="favoritesCount">
        <el-input
          v-model="queryParams.favoritesCount"
          placeholder="请输入${comment}"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-table v-loading="loading" :data="commentsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="${comment}" align="center" prop="commentId" />
      <el-table-column label="${comment}" align="center" prop="postId" />
      <el-table-column label="${comment}" align="center" prop="userId" />
      <el-table-column label="${comment}" align="center" prop="commentIndex" />
      <el-table-column label="${comment}" align="center" prop="authorName" />
      <el-table-column label="${comment}" align="center" prop="content" />
      <el-table-column label="${comment}" align="center" prop="commentTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.commentTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="${comment}" align="center" prop="likesCount" />
      <el-table-column label="${comment}" align="center" prop="favoritesCount" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="${comment}" prop="postId">
          <el-input v-model="form.postId" placeholder="请输入${comment}" />
        </el-form-item>
        <el-form-item label="${comment}" prop="userId">
          <el-input v-model="form.userId" placeholder="请输入${comment}" />
        </el-form-item>
        <el-form-item label="${comment}" prop="commentIndex">
          <el-input v-model="form.commentIndex" placeholder="请输入${comment}" />
        </el-form-item>
        <el-form-item label="${comment}" prop="authorName">
        ...
        <el-form-item label="${comment}" prop="likesCount">
          <el-input v-model="form.likesCount" placeholder="请输入${comment}" />
        </el-form-item>
        <el-form-item label="${comment}" prop="favoritesCount">
          <el-input v-model="form.favoritesCount" placeholder="请输入${comment}" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除【请填写功能名称】编号为"' + commentIds + '"的数据项？').then(function() {
